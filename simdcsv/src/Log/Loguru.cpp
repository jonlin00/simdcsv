...
Text vtextprintf(const char* format, Wikinger::fmt::format_args args) {
  return Text(STRDUP(Wikinger::fmt::vformat(format, args).c_str()));
}
#else
...
#if LOGURU_USE_FMTLIB
void vlog(Verbosity verbosity, const char* file, unsigned line, const char* format, Wikinger::fmt::format_args args) {
  auto formatted = Wikinger::fmt::vformat(format, args);
  log_to_everywhere(1, verbosity, file, line, "", formatted.c_str());
}

void raw_vlog(Verbosity verbosity, const char* file, unsigned line, const char* format, Wikinger::fmt::format_args args) {
  auto formatted = Wikinger::fmt::vformat(format, args);
...
#if LOGURU_USE_FMTLIB
void vlog_and_abort(int stack_trace_skip, const char* expr, const char* file, unsigned line, const char* format, Wikinger::fmt::format_args args) {
  auto formatted = Wikinger::fmt::vformat(format, args);
